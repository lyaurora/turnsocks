...
面板用于添加、删除、切换、备注、检查和测速 TURN 节点，也可修改 SOCKS5 监听地址、DoH 与面板登录凭据。

“检查”测量 TCP 延迟，并通过小型 HTTP 204 请求和 UDP DNS 查询验证实际 TCP / UDP 转发。“测速”只测单线程和多线程下载带宽，使用 Cloudflare 下载源，每个节点约下载 112 MiB，结果不代表所有目标站点的速度。两种结果分别保存、在同一行展示，检查与测速只更新各自的数据。

检测逐个执行，可通过“停止检测”取消当前请求和本轮剩余节点，已完成的结果会保留。“最近事件”显示最近记录的失败阶段、原因和节点切换信息；这些历史记录不代表节点当前仍有故障。

添加或删除节点时，节点池会热加载配置。修改节点备注不影响代理进程；切换节点以及修改监听地址或 DoH 时，代理服务会重启。
...
重新执行安装命令可更新二进制并重启服务。`config.env`、`turnsocks.state`、`turnsocks.tests.json` 和 `turnsocks.checks.json` 会保留。
