...
func TestLightChecksPreserveSpeedResults(t *testing.T) {
	dir := t.TempDir()
...
func TestProbeCancellationAndConcurrentRequests(t *testing.T) {
	for _, mode := range []probe.Mode{probe.ModeCheck, probe.ModeSpeed} {
		t.Run(string(mode), func(t *testing.T) {
			...
			defer cancel()
			body, _ := json.Marshal(serverRequest{Server: server, Mode: mode})
